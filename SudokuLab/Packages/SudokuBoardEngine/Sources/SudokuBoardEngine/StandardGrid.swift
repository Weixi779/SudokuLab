...
enum StandardGrid {
    ...
    static func row(forIndex index: Int) -> Int {
        index / size
    }

    static func column(forIndex index: Int) -> Int {
        index % size
    }

    static func block(row: Int, column: Int) -> Int {
        (row / blockSide) * blockSide + column / blockSide
    }
